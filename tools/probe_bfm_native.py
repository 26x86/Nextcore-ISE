#!/usr/bin/env python3
"""Exhaustive authored merging bitfield move native/reference and actual Rust provider proof."""
...
def main():
    ...
    try:
        ...
        for name in ("jit","arch","boot_jit","memory_boot","memory_boot_v2","memory_dynamic",
            ...
            objects[name]=target
        native=work/"bfm-native"
        run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2","-Wall","-Wextra","-Werror",runtime/"test_bfm_jit.c",*objects.values(),"-o",native])
        run([native])
        regression=work/"ubfm-regression"
        run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2","-Wall","-Wextra","-Werror",runtime/"test_ubfm_jit.c",*objects.values(),"-o",regression])
        run([regression])
        ...
        mutants={
            "lost_destination":("u64(c,~selected);", "u64(c,selected&0);"),
            "wrong_position":("b(c,position);}", "b(c,position^1);}"),
            "clobbered_flags":("save(c,rd,0); /* BFM preserves PSTATE and unselected destination bits. */", "save(c,rd,0);field32(c,offsetof(vf_cpu,pstate),0);"),
            "wrong_w_zero_extension":("save(c,rd,0); /* BFM preserves PSTATE and unselected destination bits. */", "if(!wide){b(c,0x49);b(c,0xb9);u64(c,UINT64_C(1)<<32);b(c,0x4c);b(c,0x09);b(c,0xc8);}save(c,rd,0);"),
            "missing_encoding_guard":("if(((w>>22)&1)!=wide || (!wide && ((r|s)&32)))", "if(0)")}
        for name,(needle,replacement) in mutants.items():
            ...
            binary=work/name
            run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2",runtime/"test_bfm_jit.c",obj,
        ...
        result=dict(schema="nextcore.bfm-native-provider-proof.v1",passed=passed,source_preserved=before==after,
