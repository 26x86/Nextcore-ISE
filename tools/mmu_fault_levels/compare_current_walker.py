...
#[path = "@EXCEPTION_PATH@"] mod exception_level;
...
def main():
 ...
 exception=source.with_name('exception_level.rs')
 before=hashlib.sha256(source.read_bytes()).hexdigest();exception_before=hashlib.sha256(exception.read_bytes()).hexdigest();sources={};vectors=[];entries=[]
 ...
 wrapper=output/'wrapper.rs';wrapper.write_text(WRAPPER.replace('@MMU_PATH@',str(source).replace('\\','\\\\').replace('"','\\"')).replace('@EXCEPTION_PATH@',str(exception).replace('\\','\\\\').replace('"','\\"')))
 ...
 after=hashlib.sha256(source.read_bytes()).hexdigest();exception_after=hashlib.sha256(exception.read_bytes()).hexdigest()
 ...
 report={'schema':'nextcore.mmu-walker-oracle-differential.v1','case_count':len(positive),'cases':positive,'negative_controls':negative,'negative_controls_detected':all(e['detected'] for e in negative),'passed':before==after and exception_before==exception_after and all(e['passed'] for e in positive) and all(e['detected'] for e in negative),'exception_source_sha256_before':exception_before,'exception_source_sha256_after':exception_after,'walker_source_sha256_before':before,'walker_source_sha256_after':after,'walker_path':str(source),'oracle_inputs':sources,'compile_command':compile_cmd,'rustc_version':subprocess.check_output([rustc,'--version'],text=True).strip(),'wrapper_sha256':hashlib.sha256(wrapper.read_bytes()).hexdigest(),'native_jit_mmu_verified':False}
