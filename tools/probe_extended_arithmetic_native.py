#!/usr/bin/env python3
"""Exhaustive authored extended arithmetic native/reference and actual Rust provider proof."""
...
def main():
    ...
    try:
        ...
        for name in ("jit","arch","boot_jit","memory_boot","memory_boot_v2","memory_dynamic",
            ...
            objects[name]=target
        native=work/"extended_arithmetic-native"
        run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2","-Wall","-Wextra","-Werror",runtime/"test_extended_arithmetic_jit.c",*objects.values(),"-o",native])
        run([native])
        ...
        mutants={
            "wrong_signed_extension":("b(c,(option&4)?0xf8:0xe8);", "b(c,0xe8);"),
            "wrong_subtraction_flags":("if(flags)save_arithmetic_flags(c,subtract); /* Extended arithmetic NZCV. */",
                "if(flags)save_arithmetic_flags(c,0); /* Extended arithmetic NZCV. */")}
        for name,(needle,replacement) in mutants.items():
            ...
            binary=work/name
            run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2",runtime/"test_extended_arithmetic_jit.c",obj,
        ...
        result=dict(schema="nextcore.extended_arithmetic-native-provider-proof.v1",passed=passed,source_preserved=before==after,
