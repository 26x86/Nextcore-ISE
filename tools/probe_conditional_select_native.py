#!/usr/bin/env python3
"""Exhaustive authored conditional selection native/reference and actual Rust provider proof."""
...
def main():
    ...
    try:
        ...
        for name in ("jit","arch","boot_jit","memory_boot","memory_boot_v2","memory_dynamic",
            ...
            objects[name]=target
        native=work/"conditional_select-native"
        run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2","-Wall","-Wextra","-Werror",runtime/"test_conditional_select_jit.c",*objects.values(),"-o",native])
        run([native])
        ...
        mutants={
            "wrong_condition":("size_t other=condition_false(c,(w>>12)&15);", "size_t other=condition_false(c,((w>>12)&15)^1);"),
            "wrong_increment":("if(w&(1u<<10)) {if(wide)b(c,0x48);b(c,0x83);b(c,0xc0);b(c,1);}",
                "if(w&(1u<<10)) {if(wide)b(c,0x48);b(c,0x83);b(c,0xc0);b(c,2);}"),
            "clobbered_pstate":("save(c,rd,0); /* Conditional selection preserves guest PSTATE. */",
                "save(c,rd,0); field32(c,offsetof(vf_cpu,pstate),0);")}
        for name,(needle,replacement) in mutants.items():
            ...
            binary=work/name
            run([args.clang,"-std=c11","-D_GNU_SOURCE","-O2",runtime/"test_conditional_select_jit.c",obj,
        ...
        result=dict(schema="nextcore.conditional_select-native-provider-proof.v1",passed=passed,source_preserved=before==after,
