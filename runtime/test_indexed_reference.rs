...
#[test]fn indexed_actual_arm_oracle() {
    for &(word,displacement,expected_x,expected_mem,expected_base) in INDEXED_ORACLE {
        ...
        cpu.x[3]=(base+512).wrapping_sub(if (word>>10)&3==3 {displacement as u64}else{0});cpu.pstate=0xb00003c5;
        ...
        assert_eq!((cpu.pc,cpu.x[3],cpu.pstate),(base+4,(base+512).wrapping_add(expected_base),0xb00003c5));
    }
}
