/* Authored bounded ISAR0 policy test, not a universal EL0 architecture claim. */
#include "jit.h"
#include <sys/mman.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(void){
 ...
 for(unsigned rd=0;rd<32;rd++)for(unsigned failure=0;failure<11;failure++){
  ...
  uint32_t word=0xd5380600|rd;if(failure==3)word^=1u<<21;if(failure==4)word^=1u<<6;if(failure==5)word^=1u<<12;if(failure==6)word^=1u<<16;
 ...
 for(unsigned el=0;el<4;el++)for(unsigned selector=0;selector<5;selector++){
  vf_cpu cpu;vf_cpu_reset(&cpu,el);if(selector==1)cpu.hcr_el2=1;if(selector==2)cpu.scr_el3=1;if(selector==3)cpu.hcr_el2=1ULL<<63;if(selector==4)cpu.scr_el3=1ULL<<63;uint64_t value=0xfeed;
  int good=el==1&&selector==0;CHECK(vf_cpu_read_sysreg(&cpu,0x4030,&value)==(good?VF_SYSREG_OK:VF_SYSREG_UNKNOWN));CHECK(value==(good?0:0xfeed));CHECK(vf_cpu_write_sysreg(&cpu,0x4030,1)==VF_SYSREG_UNKNOWN);
 }
 for(unsigned selector=0;selector<5;selector++)for(unsigned reverse=0;reverse<2;reverse++){
  vf_cpu cpu;vf_cpu_reset(&cpu,VF_EL1);uint32_t word=0xd5380600;uint8_t ram[8]={0};
  ...
  CHECK(((vf_entry)(void*)code.bytes)(&cpu,ram,8)==(reverse?VF_NEXT:VF_SYSTEM_REGISTER_TRAP));CHECK(cpu.x[0]==(reverse?0:0xfeed)&&cpu.pc==(reverse?4:0)&&cpu.retired==(reverse?1:0)&&cpu.sp==0x800&&cpu.pstate==flags);
 }

 for(unsigned i=0;i<sizeof(extension_words)/sizeof(extension_words[0]);i++){
  vf_cpu cpu;vf_cpu_reset(&cpu,VF_EL1);cpu.x[0]=11;cpu.x[1]=22;cpu.x[2]=16;cpu.sp=0x800;cpu.pstate=0xf00003c5;uint8_t ram[64]={0},before[64]={0};uint64_t regs[32];memcpy(regs,cpu.x,sizeof(regs));unsigned word=extension_words[i];int status=vf_run(&cpu,(uint8_t*)&word,4,ram,64,&code,1,perms,0);
  if(status!=VF_UNDEFINED_INSTRUCTION&&status!=VF_SYSTEM_REGISTER_TRAP){fprintf(stderr,"extension 0x%08x unexpectedly status%d\n",word,status);exit(1);}CHECK(cpu.retired==0&&cpu.pc==0&&cpu.sp==0x800&&cpu.pstate==0xf00003c5&&!memcmp(regs,cpu.x,sizeof(regs))&&!memcmp(ram,before,64));
 }
 /* ORACLE_INSERT */
