...
int main(void) {
    ...
    for(unsigned size=0;size<4;size++)for(unsigned opc=0;opc<4;opc++)for(unsigned variant=0;variant<3;variant++) {
        if(valid(size,opc)&&variant==0)continue;
        uint32_t word=instruction(size,opc,-1,3,2)|(variant==1?1u<<26:variant==2?2u<<10:0);
