...
#[test]fn mmfr0_canonical_all_forms_no_data_and_cache_equivalence(){
 let output=std::env::var("NEXTCORE_MMFR0_SNAPSHOT").unwrap();let mut snapshots=Vec::new();
 for &(word,a,b,value,nzcv) in MMFR0_ORACLE {
 ...
 for word in [0xd5380700u32,0xd5180700,0xd5380720,0xd5381700,0xd5390700]{
  for el0 in [false,true]{if word==0xd5380700&&!el0{continue;}let(c,t,mut ram,_,_)=fixture(true,&[word]);let before=ram.clone();let options=platform::BootOptionsV2{abi_version:2,struct_size:64,initial_pstate:if el0{0xf00003c0}else{0xf00003c5},..Default::default()};let(r,q)=run_with_options(c,&t,&mut ram,VA,[11,22,33,44],0,Some(&options));let b=r.base.execution.base;assert_eq!((b.status,b.retired,b.pc,b.x0,b.x1,b.x2,b.x3,q.len(),r.base.data_requests),(if el0&&word==0xd5180700 {9}else{13},0,VA,11,22,33,44,1,0),"word={word:x} el0={el0}");assert_eq!(ram,before);}
 }
