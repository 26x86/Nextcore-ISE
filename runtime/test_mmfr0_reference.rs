#[test]fn mmfr0_actual_arm_oracle(){
 for &(word,_,_,value,nzcv) in MMFR0_ORACLE {
  assert_eq!(value,if word&31==31{0}else{0x0f100005});let mut ram=[0u8;8];ram[..4].copy_from_slice(&word.to_le_bytes());let before=ram;let mut cpu=GuestCpuState::reset(0);assert_eq!(cpu.sys.id_aa64mmfr0_el1,0x0f100005);cpu.sys.id_aa64mmfr0_el1=0xfeed1234;for i in 0..31{cpu.x[i]=0x100+i as u64;}let mut expected=cpu.x;let rd=(word&31)as usize;if rd!=31{expected[rd]=value;}cpu.sp=0x12345678;cpu.pstate=0xf00003c5;
 ...
 for failure in 0..11 {let mut word=0xd5380700u32;let mut cpu=GuestCpuState::reset(0);cpu.x[0]=0xabc;cpu.sp=0x800;cpu.pstate=0xf00003c5;
