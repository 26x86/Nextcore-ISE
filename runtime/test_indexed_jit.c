/* SPDX-License-Identifier: BSD-4-Clause; authored indexed writeback proof. */
#include "jit.h"
#include <sys/mman.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
static unsigned checks,cases,mode;
...
static uint32_t instruction(unsigned size,unsigned opc,int displacement,unsigned rn,unsigned rt) {
    return 0x38000000u|(mode<<10)|(size<<30)|(opc<<22)|(((unsigned)displacement&511)<<12)|(rn<<5)|rt;
}
...
int main(void) {
    ...
    for(unsigned d=0;d<7;d++)for(unsigned sp=0;sp<2;sp++)for(unsigned target=0;target<3;target++) {
        ...
        if(!sp && rt==rn)continue;
        uint64_t base=BASE+512-(mode==3?displacements[d]:0);vf_cpu cpu;reset(&cpu);cpu.x[3]=base;cpu.sp=base;
        ...
        if(!sp)regs[3]=base+displacements[d];
        CHECK(run(&cpu,&code,instruction(size,opc,displacements[d],rn,rt),ram,sizeof(ram))==VF_BUDGET);
        CHECK(cpu.retired==1 && cpu.pc==BASE+4 && cpu.pstate==0xb00003c5 && cpu.sp==(sp?base+displacements[d]:base));
    ...
    for(unsigned fault=0;fault<3;fault++) {
        ...
        vf_cpu cpu;reset(&cpu);cpu.x[3]=address+(mode==3?256:0);uint64_t oldbase=cpu.x[3],oldrt=cpu.x[2];uint8_t ram[512],before[512];memset(ram,0xa5,512);memcpy(before,ram,512);
        CHECK(run(&cpu,&code,instruction(size,opc,-256,3,2),ram,512)==(fault==2?VF_ALIGNMENT_FAULT:VF_DATA_ABORT));
        CHECK(cpu.pc==BASE && cpu.retired==0 && cpu.far==address && cpu.x[3]==oldbase && cpu.x[2]==oldrt && !memcmp(ram,before,512));
        CHECK(cpu.esr==(0x96000000u|(opc?0:64)|(fault==2?0x21:7)));
    }
    for(unsigned size=0;size<4;size++)for(unsigned opc=0;opc<4;opc++)for(unsigned variant=0;variant<4;variant++) {
        if(valid(size,opc)&&variant==0)continue;
        uint32_t word=instruction(size,opc,-1,3,variant==3?3:2);
        if(variant==1)word|=1u<<26;if(variant==2)word=(word&~(3u<<10))|(2u<<10);
        vf_cpu cpu;reset(&cpu);cpu.x[3]=BASE+128;uint64_t regs[32];memcpy(regs,cpu.x,sizeof(regs));uint8_t ram[512],before[512];memset(ram,0xa5,512);memcpy(before,ram,512);
        CHECK(run(&cpu,&code,word,ram,512)==VF_UNDEFINED_INSTRUCTION);
        CHECK(cpu.retired==0 && cpu.pc==BASE && !memcmp(ram,before,512) && !memcmp(cpu.x,regs,sizeof(regs)));
    }
    for(unsigned size=0;size<4;size++)for(unsigned opc=0;opc<4;opc++)if(valid(size,opc)) {
