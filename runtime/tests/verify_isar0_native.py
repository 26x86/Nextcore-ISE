#!/usr/bin/env python3
"""Authored isar0 scalar actual native, Arm oracle and canonical memory proof."""
...
def main():
 ...
 for rd in range(32):
  word=0xd5380600|rd
  asm+=load('x4',0xf0000000)+['msr nzcv,x4',f'mrs {"x"+str(rd) if rd!=31 else "xzr"},ID_AA64ISAR0_EL1',f'mov x4,{"x"+str(rd) if rd!=31 else "xzr"}']+load('x18',0x40100000+rd*16)+['str x4,[x18]','mrs x4,nzcv','str x4,[x18,#8]']
 ...
 for i,(word,a0,b0) in enumerate(vectors):
  ...
  assert hardware_value==(0 if (word&31)==31 else 0x11120) and nzcv==0xf0000000
  value=0 # Explicit software policy; not the observed hardware feature value.
 ...
 extensions=['aese v0.16b,v1.16b','sha1c q0,s1,v2.4s','sha256h q0,q1,v2.4s','crc32x w0,w1,x2','ldadd x0,x1,[x2]','cas x0,x1,[x2]','sqrdmlah v0.4s,v1.4s,v2.4s','eor3 v0.16b,v1.16b,v2.16b,v3.16b','sm3ss1 v0.4s,v1.4s,v2.4s,v3.4s','sm4e v0.4s,v1.4s','sdot v0.4s,v1.16b,v2.16b','fmlal v0.2s,v1.2h,v2.2h','cfinv','tlbi vae1os,x0','mrs x0,rndr','tstart x0']
 (out/'extensions.S').write_text('.text\n'+ '\n'.join(extensions)+'\n')
 run(['clang-18','--target=aarch64-none-elf','-march=armv8.6-a+crypto+sha3+sm4+fp16fml+rng+tme','-c',out/'extensions.S','-o',out/'extensions.o'],'extensions-build.log')
 ...
 c=(r/'test_isar0_jit.c').read_text()
 ...
 oracle_rust='const EXTENSION_WORDS:&[u32]=&['+','.join(str(w) for w in extension_words)+'];\n'+'const ISAR0_ORACLE:&[(u32,u64,u64,u64,u64)]=&['+','.join(f'({word},{a0},{b0},{0},{values[i*2+1]})' for i,(word,a0,b0) in enumerate(vectors))+'];\n'
 ...
 extra=(r/'test_isar0_provider.rs').read_text()
 (out/'provider.rs').write_text(base+'\n'+oracle_rust+extra)
 run(['rustc','--edition=2021','--test','-Copt-level=2',out/'provider.rs','--extern','nextcore_memory_service='+str(service),'-o',out/'provider',*['-Clink-arg='+str(x) for x in objects]],'provider-build.log');os.environ['NEXTCORE_ISAR0_SNAPSHOT']=str(out/'cached.snapshot');run([out/'provider','--test-threads=1'],'provider.log')
 (out/'arch-reference.rs').write_text((r/'preos/src/arch.rs').read_text().split('#[cfg(test)]',1)[0]+'\n'+oracle_rust+(r/'test_isar0_reference.rs').read_text())
 ...
 for mode,flags in [('uncached',['-DNEXTCORE_DISABLE_PROVIDER_CACHE']),('small-slot',['-DNEXTCORE_PROVIDER_CACHE_SLOT_BYTES=64'])]:
  ...
  run(['rustc','--edition=2021','--test','-Copt-level=2',out/'provider.rs','--extern','nextcore_memory_service='+str(service),'-o',out/mode,'-Clink-arg='+str(obj),*['-Clink-arg='+str(x) for x in objects[1:]]],mode+'-link.log');os.environ['NEXTCORE_ISAR0_SNAPSHOT']=str(out/(mode+'.snapshot'));run([out/mode,'--test-threads=1'],mode+'.log');assert (out/(mode+'.snapshot')).read_bytes()==(out/'cached.snapshot').read_bytes()
 assert before=={str(x.relative_to(r)):sha(x) for x in files}
 receipt=dict(passed=True,extension_words=dict(zip(extensions,[hex(w) for w in extension_words])),register="ID_AA64ISAR0_EL1",destination_registers=32,arm_cpu="cortex-a72",hardware_isar0="0x11120",software_policy_isar0=0,hardware_value_equals_software_policy=False,oracle_scope="Encoding/access/XZR/NZCV semantics only; feature values deliberately differ",exposed_result_requests_ram_equal=True,native_modes=['cached','uncached','small-slot'],reference_oracle_vectors=len(vectors),process_reaped=True,oracle_vectors=len(vectors),qemu_sha256=sha(pathlib.Path(qemu)),qemu_version=subprocess.check_output([qemu,'--version'],text=True).splitlines()[0],source_sha256=before,sources_preserved=True,commands=commands,original_images_used=False,physical_boot_verified=False)
