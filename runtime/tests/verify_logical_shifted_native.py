#!/usr/bin/env python3
"""Authored logical_shifted scalar actual native, Arm oracle and canonical memory proof."""
...
def main():
 ...
 names=['and','bic','orr','orn','eor','eon','ands','bics'];shifts=['lsl','lsr','asr','ror']
 for width in [32,64]:
  reg='w' if width==32 else 'x'
  for op,name in enumerate(names):
   for shift,shiftname in enumerate(shifts):
    for amount in [0,1,width//2,width-1]:
     for a0,b0 in [(0x91a2b3c4d5e6f780,0x8123456789abcdef),(0,0),((1<<64)-1,(1<<64)-1)]:
      word=0x0a000000|(int(width==64)<<31)|((op>>1)<<29)|((op&1)<<21)|(shift<<22)|(1<<16)|(amount<<10)|2
      asm+=load('x0',a0)+load('x1',b0)+load('x4',0xf0000000)+['msr nzcv,x4',f'{name} {reg}2,{reg}0,{reg}1,{shiftname} #{amount}','str x2,[x18],#8','mrs x4,nzcv','str x4,[x18],#8']
      vectors.append((word,a0,b0))
 ...
 c=(r/'test_logical_shifted_jit.c').read_text()
 ...
 CHECK(cpu.x[0]==oracle[i].a && cpu.x[1]==oracle[i].b && cpu.x[2]==oracle[i].value && cpu.pstate==(oracle[i].nzcv|0x3c5) && cpu.sp==0x12345678 && cpu.pc==4 && cpu.retired==1);
 }"""
 ...
 oracle_rust='const LOGICAL_ORACLE:&[(u32,u64,u64,u64,u64)]=&['+','.join(f'({word},{a0},{b0},{values[i*2]},{values[i*2+1]})' for i,(word,a0,b0) in enumerate(vectors))+'];\n'
 ...
 extra=(r/'test_logical_shifted_provider.rs').read_text()
 (out/'provider.rs').write_text(base+'\n'+oracle_rust+extra)
 run(['rustc','--edition=2021','--test','-Copt-level=2',out/'provider.rs','--extern','nextcore_memory_service='+str(service),'-o',out/'provider',*['-Clink-arg='+str(x) for x in objects]],'provider-build.log');os.environ['NEXTCORE_LOGICAL_SNAPSHOT']=str(out/'cached.snapshot');run([out/'provider','logical_shifted_canonical','--test-threads=1'],'provider.log')
 (out/'arch-reference.rs').write_text((r/'preos/src/arch.rs').read_text().split('#[cfg(test)]',1)[0]+'\n'+oracle_rust+(r/'test_logical_shifted_reference.rs').read_text())
 ...
 run(['rustc','--edition=2021','--test','-Copt-level=2',out/'reference.rs','-o',out/'reference'],'reference-build.log');run([out/'reference','logical_shifted_actual_arm_oracle'],'reference.log')
 for mode,flags in [('uncached',['-DNEXTCORE_DISABLE_PROVIDER_CACHE']),('small-slot',['-DNEXTCORE_PROVIDER_CACHE_SLOT_BYTES=64'])]:
  ...
  run(['rustc','--edition=2021','--test','-Copt-level=2',out/'provider.rs','--extern','nextcore_memory_service='+str(service),'-o',out/mode,'-Clink-arg='+str(obj),*['-Clink-arg='+str(x) for x in objects[1:]]],mode+'-link.log');os.environ['NEXTCORE_LOGICAL_SNAPSHOT']=str(out/(mode+'.snapshot'));run([out/mode,'logical_shifted_canonical','--test-threads=1'],mode+'.log');assert (out/(mode+'.snapshot')).read_bytes()==(out/'cached.snapshot').read_bytes()
 assert before=={str(x.relative_to(r)):sha(x) for x in files}
 receipt=dict(passed=True,operations=8,widths=[32,64],shifts=4,native_modes=['cached','uncached','small-slot'],reference_oracle_vectors=len(vectors),process_reaped=True,oracle_vectors=len(vectors),qemu_sha256=sha(pathlib.Path(qemu)),qemu_version=subprocess.check_output([qemu,'--version'],text=True).splitlines()[0],source_sha256=before,sources_preserved=True,commands=commands,original_images_used=False,physical_boot_verified=False)
