#!/usr/bin/env python3
"""Authored indexed scalar actual native, Arm oracle and canonical memory proof."""
...
def main():
 ...
 forms=[('strb','w',0,0),('ldrb','w',0,1),('ldrsb','x',0,2),('ldrsb','w',0,3),('strh','w',1,0),('ldrh','w',1,1),('ldrsh','x',1,2),('ldrsh','w',1,3),('str','w',2,0),('ldr','w',2,1),('ldrsw','x',2,2),('str','x',3,0),('ldr','x',3,1)]
 ...
 for mnemonic,reg,size,opc in forms:
  for mode in [1,3]:
   for disp in [-256,-1,255]:
    word=0x38000000|size<<30|opc<<22|(disp&511)<<12|mode<<10|3<<5|2
    address=f"{'sub' if disp>0 else 'add'} x3,x19,#{abs(disp)}" if mode==3 else 'mov x3,x19'
    operand=f'[x3,#{disp}]!' if mode==3 else f'[x3],#{disp}'
    asm+=['mov x2,#0x7788','movk x2,#0x5566,lsl #16','movk x2,#0x3344,lsl #32','movk x2,#0x1122,lsl #48','mov x4,#0x8081','movk x4,#0x8283,lsl #16','movk x4,#0x8485,lsl #32','movk x4,#0x8687,lsl #48','str x4,[x19]',address,f'{mnemonic} {reg}2,{operand}','str x2,[x18],#8','ldr x4,[x19]','str x4,[x18],#8','sub x4,x3,x19','str x4,[x18],#8']
 ...
 for i,(word,disp) in enumerate(vectors):
  x,mem,base=values[i*3:i*3+3];assert base==(disp if (word>>10)&3==1 else 0)&((1<<64)-1)
  header.append(f'{{0x{word:x}u,{disp},0x{x:x}ULL,0x{mem:x}ULL,0x{base:x}ULL}}')
 (out/'oracle.h').write_text('static const struct {unsigned word;int disp;unsigned long long x,mem,base;} oracle[]={'+','.join(header)+'};\n')
 objects=[]
 ...
 extra=(r/'test_indexed_provider.rs').read_text()
 (out/'provider.rs').write_text(base+'\n'+extra)
 run(['rustc','--edition=2021','--test','-Copt-level=2',out/'provider.rs','--extern','nextcore_memory_service='+str(service),'-o',out/'provider',*['-Clink-arg='+str(x) for x in objects]],'provider-build.log');os.environ['NEXTCORE_INDEXED_SNAPSHOT']=str(out/'cached.snapshot');run([out/'provider','--test-threads=1'],'provider.log')
 oracle_rust='const INDEXED_ORACLE:&[(u32,i64,u64,u64,u64)]=&['+','.join(f'({word},{disp},{values[i*3]},{values[i*3+1]},{values[i*3+2]})' for i,(word,disp) in enumerate(vectors))+'];\n'
 (out/'arch-reference.rs').write_text((r/'preos/src/arch.rs').read_text().split('#[cfg(test)]',1)[0]+'\n'+oracle_rust+(r/'test_indexed_reference.rs').read_text())
 ...
 run(['rustc','--edition=2021','--test','-Copt-level=2',out/'reference.rs','-o',out/'reference'],'reference-build.log');run([out/'reference','indexed_actual_arm_oracle'],'reference.log')
 for mode,flags in [('uncached',['-DNEXTCORE_DISABLE_PROVIDER_CACHE']),('small-slot',['-DNEXTCORE_PROVIDER_CACHE_SLOT_BYTES=64'])]:
  ...
  run(['rustc','--edition=2021','--test','-Copt-level=2',out/'provider.rs','--extern','nextcore_memory_service='+str(service),'-o',out/mode,'-Clink-arg='+str(obj),*['-Clink-arg='+str(x) for x in objects[1:]]],mode+'-link.log');os.environ['NEXTCORE_INDEXED_SNAPSHOT']=str(out/(mode+'.snapshot'));run([out/mode,'--test-threads=1'],mode+'.log')
 assert (out/'cached.snapshot').read_bytes()==(out/'uncached.snapshot').read_bytes()==(out/'small-slot.snapshot').read_bytes()
 assert before=={str(x.relative_to(r)):sha(x) for x in files}
 receipt=dict(passed=True,exposed_result_requests_ram_equal=True,cache_snapshot_sha256=sha(out/'cached.snapshot'),forms=13,native_modes=['cached','uncached','small-slot'],reference_oracle_vectors=len(vectors),process_reaped=True,oracle_vectors=len(vectors),qemu_sha256=sha(pathlib.Path(qemu)),qemu_version=subprocess.check_output([qemu,'--version'],text=True).splitlines()[0],source_sha256=before,sources_preserved=True,commands=commands,original_images_used=False,physical_boot_verified=False)
