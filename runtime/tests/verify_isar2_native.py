...
def main():
 ...
 for rd in range(32):
  word=0xd5380640|rd
  asm+=load('x4',0xf0000000)+['msr nzcv,x4',f'mrs {"x"+str(rd) if rd!=31 else "xzr"},ID_AA64ISAR2_EL1',f'mov x4,{"x"+str(rd) if rd!=31 else "xzr"}']+load('x18',0x40100000+rd*16)+['str x4,[x18]','mrs x4,nzcv','str x4,[x18,#8]']
 ...
 for i,(word,a0,b0) in enumerate(vectors):
  ...
  assert hardware_value==(0 if (word&31)==31 else 0) and nzcv==0xf0000000
  value=0 # Exact observed Cortex-A72 value, also the independently selected bounded policy.
 ...
 extensions=['wfet x0','wfit x0','bc.eq #4','cpyfp [x0]!,[x1]!,x2!','setp [x0]!,x1!,x2','rprfm pldkeep,x0,[x1]']
 (out/'extensions.S').write_text('.text\n'+ '\n'.join(extensions)+'\n')
 run(['clang-18','--target=aarch64-none-elf','-march=armv8.8-a+mops','-c',out/'extensions.S','-o',out/'extensions.o'],'extensions-build.log')
