...
#[test]fn isar2_canonical_all_forms_no_data_and_cache_equivalence(){
 let output=std::env::var("NEXTCORE_ISAR2_SNAPSHOT").unwrap();let mut snapshots=Vec::new();
 for &(word,a,b,value,nzcv) in ISAR2_ORACLE {
...
#[test]fn isar2_extension_policy_provider_rejects(){for &word in EXTENSION_WORDS{let(mut c,t,mut ram,_,_)=fixture(true,&[word]);c.profile=3;c.sctlr=0x30d00801;let before=ram.clone();let(r,q)=run(c,&t,&mut ram,VA,[11,22,VA+32,44],0);let b=r.base.execution.base;assert!(b.status==8||b.status==13,"extension {word:x} status{}",b.status);assert_eq!((b.retired,b.pc,b.x0,b.x1,b.x2,b.x3,r.base.data_requests,q.len()),(0,VA,11,22,VA+32,44,0,1));assert_eq!(ram,before);}}
