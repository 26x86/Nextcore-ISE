...
#[test]fn isar0_canonical_all_forms_no_data_and_cache_equivalence(){
 let output=std::env::var("NEXTCORE_ISAR0_SNAPSHOT").unwrap();let mut snapshots=Vec::new();
 for &(word,a,b,value,nzcv) in ISAR0_ORACLE {
 ...
 for word in [0xd5380600u32,0xd5180600,0xd5380640,0xd5381600,0xd5390600]{
  for el0 in [false,true]{if word==0xd5380600&&!el0{continue;}let(c,t,mut ram,_,_)=fixture(true,&[word]);let before=ram.clone();let options=platform::BootOptionsV2{abi_version:2,struct_size:64,initial_pstate:if el0{0xf00003c0}else{0xf00003c5},..Default::default()};let(r,q)=run_with_options(c,&t,&mut ram,VA,[11,22,33,44],0,Some(&options));let b=r.base.execution.base;assert_eq!((b.status,b.retired,b.pc,b.x0,b.x1,b.x2,b.x3,q.len(),r.base.data_requests),(13,0,VA,11,22,33,44,1,0));assert_eq!(ram,before);}
 }
 std::fs::write(output,snapshots).unwrap();
}

#[test]fn isar0_extension_policy_provider_rejects(){for &word in EXTENSION_WORDS{let(mut c,t,mut ram,_,_)=fixture(true,&[word]);c.profile=3;c.sctlr=0x30d00801;let before=ram.clone();let(r,q)=run(c,&t,&mut ram,VA,[11,22,VA+32,44],0);let b=r.base.execution.base;assert!(b.status==8||b.status==13,"extension {word:x} status{}",b.status);assert_eq!((b.retired,b.pc,b.x0,b.x1,b.x2,b.x3,r.base.data_requests,q.len()),(0,VA,11,22,VA+32,44,0,1));assert_eq!(ram,before);}}
