/* Authored bounded ISAR2 policy test, not a universal EL0 architecture claim. */
#include "jit.h"
#include <sys/mman.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(void){
 ...
 for(unsigned rd=0;rd<32;rd++)for(unsigned failure=0;failure<11;failure++){
  ...
  uint32_t word=0xd5380640|rd;if(failure==3)word^=1u<<21;if(failure==4)word^=1u<<5;if(failure==5)word^=1u<<12;if(failure==6)word^=1u<<16;
 ...
 for(unsigned el=0;el<4;el++)for(unsigned selector=0;selector<5;selector++){
  ...
  int good=el==1&&selector==0;CHECK(vf_cpu_read_sysreg(&cpu,0x4032,&value)==(good?VF_SYSREG_OK:VF_SYSREG_UNKNOWN));CHECK(value==(good?0:0xfeed));CHECK(vf_cpu_write_sysreg(&cpu,0x4032,1)==VF_SYSREG_UNKNOWN);
 }
 for(unsigned selector=0;selector<5;selector++)for(unsigned reverse=0;reverse<2;reverse++){
  vf_cpu cpu;vf_cpu_reset(&cpu,VF_EL1);uint32_t word=0xd5380640;uint8_t ram[8]={0};
